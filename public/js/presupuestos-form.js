(function ($) {
	...
	var state = window.presupuestoFormState || {};
	...
	function renderCatalogo() {
		...
		var filtroTipo = $('.presupuesto-catalogo-filtros .btn.active').data('filtro-tipo') || 'todos';
		...
		if (!articulos.length) {
			...
			articulos.forEach(function (articulo) {
				...
				html += '' +
					'<div class="catalogo-item" data-articulo-id="' + articulo.id + '" style="--presupuesto-accent: ' + acento + ';">' +
	...
	$(function () {
		...
		$('.presupuesto-catalogo-filtros').on('click', '.btn', function () {
			$('.presupuesto-catalogo-filtros .btn').removeClass('active');
		...
		// Receptor mutuamente excluyente: elegir cliente limpia el lead y viceversa (el
		// presupuesto solo admite uno de los dos, ver StorePresupuestoRequest).
		...
		$('#presupuesto-form').on('submit', function () {
			var $form = $(this);
