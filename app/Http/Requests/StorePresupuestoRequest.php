...
class StorePresupuestoRequest extends FormRequest
{
    ...
    public function withValidator(ValidatorContract $validator): void
    {
        $validator->after(function (ValidatorContract $validator) {
            ...
            if ($this->input('oportunidad_id')) {
                $oportunidad = Oportunidad::find($this->input('oportunidad_id'));

                if ($oportunidad && $oportunidad->etapa->value === 'perdida') {
                    $validator->errors()->add('oportunidad_id', 'No se pueden crear presupuestos para una oportunidad perdida.');
                }
            }
        });
    }
